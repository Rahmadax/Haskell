...
module Assignment2 where 
  ...
  import Data.List
   ...
   tar - taget index 
   index - the place of an element in a list
   maxDs - Total number of Dominoes in the game
   dsPHand - Number of Domino per hand in 2 player game
  ...
  simplePlayer hand board = simplePlayerA (possPlays board hand) -- checks poss plays (A1)
  ...
  playDomsHandler (dp1,h1,s1) (dp2,h2,s2) b _
    ...
    | knockingP b h1 = playDomsHandler (dp1,h1,s1) (dp2,h2,nScore) nBoard 2
    where (nBoard, nScore) = nextPlay (dp2,h2,s2) b

  -- if no knocking, the called player (passed Int) takes their turn and then calls for
  -- the game handler again with the other players turn
  -- player 1 called
  ...
  nextPlay (dp,hand,score) b = (nBoard, (score+scoreBoard nBoard))
    where nBoard = (nextPlayDom (dp hand b) b) 
